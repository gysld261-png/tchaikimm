...
두 패널이 맞닿은 실제 선이 기준이 됩니다. **덤으로 이력(hysteresis)이 생깁니다** —
A가 활성이면 경계가 1344px로, B가 활성이면 576px로 **활성 패널에서 멀어지므로**
경계 근처에서 깜빡이지 않고, 한 번 넓어진 패널은 그 밖으로 나가야 바뀝니다.

### 검증 (Playwright + Chromium, 1920 × 1080)

CDN이 막힌 환경이라 GSAP 최소 스텁을 넣어 실제 코드 경로를 돌렸습니다.

| 마우스 위치 | 패널폭 A/B | 경계 | 영상 opacity (bespoke/shop) |
|---|---|---|---|
| 초기 | 960 / 960 | 960 | 0 / 0 |
| x=1500 (shop 쪽) | 576 / 1344 | 576 | 0 / **1** |
| **x=716 (shop 버튼)** | 576 / 1344 | 576 | 0 / **1** ✓ 유지 |
| x=200 (bespoke 버튼) | 1344 / 576 | 1344 | **1** / 0 ✓ 유지 |

버튼 중심에서 `document.elementFromPoint`가 `hero_panel_button`을 반환 —
**실제로 클릭 가능한 상태**임을 확인했습니다.
