...
## main 히어로 — 버튼에 다가가면 반대쪽 영상이 켜지던 문제 (2026-08-09)

두 패널(bespoke / shop) 중 한쪽 버튼을 누르러 가면 반대편 영상으로 뒤집히던
문제입니다. **`pages/main/js/main.js`의 `handleHeroPointerMove` 두 줄만** 고쳤습니다.

### 원인 — 경계가 "화면 절반"에 고정돼 있었습니다

```js
var half = x < rect.width / 2 ? panelA : panelB;   // 항상 960px 기준
```

패널은 호버하면 **70% / 30%로 넓어졌다 좁아지는데** 판정 기준선만 50%에
멈춰 있었습니다. 실측(1920 기준):

| | 아무것도 활성 아님 | shop 활성 |
|---|---|---|
| shop 패널 시작 | 960px | **576px** |
| **shop 버튼 위치** | 1040px | **656px** |
| 옛 기준선 | 960px | 960px (그대로) |
| 판정 | shop ✓ | **bespoke ✗ (뒤집힘)** |

shop 패널이 넓어지면 그 안의 버튼(패널 왼쪽에서 80px)이 **화면 절반보다
왼쪽으로 들어옵니다.** 버튼에 다가가는 동작이 곧 "왼쪽으로 이동"이라 매번
bespoke로 넘어갔습니다.

### 고친 방법 — "다 늘어났을 때의 폭"을 경계로 씁니다
...
**★ 지금 그려진 폭(`getBoundingClientRect`)이 아니라 `targetRatio`입니다.**
폭은 `SMOOTH_LERP`로 서서히 자라므로, 막 진입한 순간 경계는 아직 50% 근처에
있습니다. 넓어지는 영역 안쪽으로 **빠르게** 마우스를 옮기면 아직 도착하지 않은
경계를 앞질러 넘어가 버립니다. `targetRatio`를 쓰면 활성 패널이 정해지는 즉시
경계가 최종 위치로 옮겨가서, **애니메이션 도중에도 "다 늘어나는 곳까지" 어디를
지나든 반대쪽으로 넘어가지 않습니다.**

경계가 활성 패널에서 멀어지는 방향이라 이력(hysteresis)도 함께 생깁니다 —
bespoke 활성이면 1344px, shop 활성이면 576px(1920 기준). 한 번 열린 패널은
그 밖으로 확실히 나가야 바뀝니다.

### 전환 속도를 늦췄습니다

"휙휙 넘어간다"는 지적에 따라 조정했습니다. 값은 함수 위 주석에 실측 표와 함께
모여 있습니다.

| 상수 | 이전 | 현재 | 의미 |
|---|---|---|---|
| `SMOOTH_LERP` | 0.028 | **0.018** | 폭 전환. 90%까지 1.37초 → **2.12초** |
| `FADE_DURATION` | 0.9 | **1.4** | 사진↔영상 겹침(초) |

**★ 지수 감속이라 "완전 정착"까지는 훨씬 오래 걸립니다**(0.018에서 4.6초).
끝으로 갈수록 1px 단위로 움직여 눈에 띄지 않으므로, **"90%까지" 시간이 실제
체감 전환 시간**입니다. 주석의 표도 이 기준으로 적었습니다.

### 검증 (Playwright + Chromium, 1920 × 1080)

CDN이 막힌 환경이라 GSAP 최소 스텁을 넣어 실제 코드 경로를 돌렸습니다.

**버튼 위치** — shop 버튼 중심에서 `document.elementFromPoint`가
`hero_panel_button`을 반환하고 shop 영상이 유지됩니다(실제로 클릭 가능).

**넓어지는 영역 전체 유지** — 폭이 아직 960px(전환 전)인 상태에서 마우스를
안쪽으로 빠르게 옮겨도 활성 패널이 유지됩니다. 경계가 그려진 폭이 아니라
`targetRatio`에서 오므로 가능한 결과입니다.

| bespoke 진입 후 x | 활성 | | shop 진입 후 x | 활성 |
|---|---|---|---|---|
| 700 | bespoke ✓ | | 1300 | shop ✓ |
| 900 | bespoke ✓ | | 1000 | shop ✓ |
| 1100 | bespoke ✓ | | 800 | shop ✓ |
| **1300** | bespoke ✓ | | **620** | shop ✓ |

경계를 확실히 넘으면(1700 → 200) 정상적으로 전환됩니다.

**전환 속도** — panelA 폭 960 → 576 진행: 0.65초에 797px, 1.30초에 685px,
2.12초에 621px.

### 확인하지 못한 부분

- **실제 마우스로 움직였을 때의 감각**은 사용자가 봐야 합니다. 좌표를 찍어
  확인한 것이라 전환이 자연스러운지는 판단할 수 없습니다.
- GSAP 스텁으로 돌린 것이라 **폭 전환 애니메이션의 실제 속도감**은 보지
  못했습니다(스텁은 최종 상태만 적용합니다).
