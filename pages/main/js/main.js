...
(function () {
  ...
  (function setupHeroHoverFill() {
    ...
    /* ★ 히어로 두 패널의 전환 감각 — 이 다섯 숫자로 조정합니다.

       EXPANDED_RATIO  활성 패널이 차지하는 비율(0.7 = 70%). 반대쪽은 30%가 됩니다.
       NEUTRAL_RATIO   아무 패널도 활성이 아닐 때의 비율(0.5 = 반반)

       SMOOTH_LERP     폭이 바뀌는 속도. 매 프레임 목표까지 남은 거리의 이 비율만큼
                       움직입니다. **낮을수록 천천히** 바뀝니다.
                       60fps에서 실측한 시간입니다(절반까지 / 90%까지):

                         0.05   0.23초 / 0.75초   빠릿함
                         0.028  0.42초 / 1.37초   이전 값
                         0.018  0.65초 / 2.12초   ← 현재 값
                         0.008  1.45초 / 4.78초   아주 느림

                       ※ 0.008 아래로는 권하지 않습니다. 마우스를 옮겨도 한참
                         가만히 있는 것처럼 보여 고장으로 느껴집니다.
                       ※ 지수 감속이라 끝으로 갈수록 아주 잘게 움직입니다.
                         "90%까지" 시간이 눈으로 보는 전환 시간에 가깝습니다.

       SETTLE_THRESHOLD 목표에 이만큼 가까워지면 멈춥니다. 건드릴 일이 거의 없습니다.

       FADE_DURATION   사진 ↔ 영상이 겹쳐 바뀌는 시간(초). **길수록 부드럽게** 섞입니다.
                         0.9 이전 값  /  1.4 현재 값  /  2.0 아주 느림
                       ※ 폭 전환(90% 기준 2.12초)보다 조금 짧게 두는 편이 좋습니다.
                         더 길면 패널이 다 자리 잡은 뒤에도 영상이 흐릿하게 남습니다. */
    var EXPANDED_RATIO = 0.7;
    var NEUTRAL_RATIO = 0.5;
    var SMOOTH_LERP = 0.018;
    var SETTLE_THRESHOLD = 0.0008;
    var FADE_DURATION = 1.4;
    ...
    gsap.matchMedia().add("(min-width: 768px) and (hover: hover) and (prefers-reduced-motion: no-preference)", function () {
      ...
      /* ★ 경계는 "화면 절반"도 아니고 "지금 그려진 폭"도 아닌,
         **다 늘어났을 때의 폭(targetRatio)** 입니다.

         ① 왜 화면 절반이면 안 되나
            패널은 호버하면 70% / 30%로 넓어졌다 좁아집니다. shop 패널이 넓어지면
            그 안의 버튼(패널 왼쪽에서 80px)이 화면 절반보다 왼쪽으로 들어옵니다
            (1920 기준 버튼 x가 1040 → 656). 그래서 버튼을 누르러 가는 동작이 곧
            "왼쪽으로 이동"이 되어 매번 반대편으로 뒤집혔습니다.

         ② 왜 "지금 그려진 폭"으로도 부족한가
            폭은 SMOOTH_LERP로 서서히 자랍니다. 막 진입한 순간 경계는 아직
            50% 근처에 있어서, 넓어지는 영역 안쪽으로 **빠르게** 마우스를 옮기면
            아직 도착하지 않은 경계를 앞질러 넘어가 버립니다.

         ③ 그래서 targetRatio를 씁니다
            활성 패널이 정해지는 즉시 경계가 최종 위치(70% 또는 30%)로 옮겨갑니다.
            애니메이션이 끝나기를 기다리지 않으므로, **"다 늘어나는 곳까지"
            어디를 지나도 반대쪽으로 넘어가지 않습니다.**

            경계가 활성 패널에서 멀어지는 방향이라 이력(hysteresis)도 함께
            생깁니다 — bespoke 활성이면 경계 1344px, shop 활성이면 576px(1920 기준).
            한 번 열린 패널은 그 밖으로 확실히 나가야 바뀝니다. */
      function handleHeroPointerMove(event) {
        var rect = hero.getBoundingClientRect();
        var boundary = rect.left + rect.width * targetRatio;
