...
- **인트로(pin)**: `.showcase`를 `start: "top top"`, `end: "+=100%"`, `pin: true`, `scrub: 1`로 고정.
  화면 한 장 분량(1000px)을 스크롤하는 동안 하나의 `gsap.timeline()`이 진행됩니다.
  - 상단 두 장(`.showcase_photo_a/_b`): 가운데로 모이며(x ±304) `scale 1 → 1.35`,
    `opacity 1 → 0.12`, `blur 0 → 14px`. 이동량은 하드코딩이 아니라
    `(960 - 카드중심x) * CONVERGE_RATIO(0.45)`로 계산해 시안 좌표가 바뀌어도 따라갑니다.
  - 기울기는 안쪽 `img`의 `rotation`을 -15.55° → -4°, 13.07° → 3.5°로 완화합니다.
  - 가운데 `h2`의 단어 7개가 `y: 42 → 0`, `opacity: 0 → 1`로 `stagger: 0.08` 순차 등장.
...
1. **`pinnedContainer: .showcase`가 없으면 갤러리가 깨집니다.**
   ...
   여백(100vh)만큼 시작 지점이 앞으로 당겨집니다. 실제로 처음엔 카드 c/d/e의 트리거가
   1472/1735/1907에서 시작해 pin 구간(1182~2182) 안에 묻혔습니다.
   지정 후 2472/2735/2907로 교정돼 pin이 끝난 뒤 시작합니다.
...
4. **pin 때문에 페이지가 100vh 길어집니다.** showcase 아래 섹션의 문서 좌표가 그만큼 밀립니다
   (1920×1000 기준 archive 4517 → 5517, asworn 7272 → 8272, footer 8781 → 9781).
   섹션 내부 좌표와 높이는 그대로입니다.

#### 검증 결과 (1920 × 1000)

- 트리거 6개 생성. pin 1182→2182 구간에서 `position: fixed` 확인,
  progress가 0 / 0.25 / 0.5 / 0.75 / 1로 선형 추적.
- 타임라인 값 직접 확인 — progress 0: x0 · scale 1 · opacity 1 · blur 0 · rotation -15.55/13.07,
  단어 `y42 opacity0`. progress 0.5: x ±179 · scale 1.21 · opacity 0.48 · blur 8.26px ·
  rotation -8.74/7.42, 첫 단어만 안착(stagger 동작). progress 1: x +304/-303(좌우 대칭) ·
  scale 1.35 · opacity 0.12 · blur 14px · rotation -4/3.5, 단어 전부 안착.
...
- `CONVERGE_RATIO` — 상단 카드가 중앙까지 가는 비율. 1이면 두 장이 겹칩니다. (현재 0.45)
- `RISE_BASE` / `RISE_STEP` — 갤러리 카드가 떠오르는 기본 거리와 카드별 증가폭. (현재 110 / 26)
- pin 길이는 `buildIntroTimeline`의 `end: "+=100%"`. 늘리면 인트로가 더 천천히 진행됩니다.
...
2. 셀럽 캐러셀 인터랙션
3. 나머지 Figma 노드의 화면 정체 확인 후 페이지 구현
4. Collection 반응형(360 / 768 / 1280) 대응
5. Collection hero 영상 편집본이 나오면 같은 명령으로 재압축 후 `collection_film.mp4` 교체
