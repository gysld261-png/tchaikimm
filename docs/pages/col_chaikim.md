...
`test/col_chaikim/` 페이지의 작업 내역입니다.
대상 화면은 Figma `4조 한복판 - 차이킴` node `1710:3971`("col_chaikim", 1920 × 9796.75)입니다.

자매 페이지 `test/col_chaikimyoungjin/`(node `1710:3865`)와 **레이아웃·class 이름·인터랙션이 같습니다.**
...
| `test/collection_test/css/collection.css` | `test/col_chaikimyoungjin/css/col_chaikimyoungjin.css` |
| `test/collection_test/js/collection.js` | `test/col_chaikimyoungjin/js/col_chaikimyoungjin.js` |
| `/asset/collection/` (70개, 220MB+) | 각 페이지 폴더 안의 `asset/` 두 개로 분리 |
| — | `test/col_chaikim/` (이번에 새로 만든 페이지) |
...
- `test/col_chaikim/asset/` — 이 시안이 쓰는 사진. Figma에서 내려받은 17장과
  저장소에 이미 있던 `archive_kim20xx_*` 내보내기입니다.
- `test/col_chaikimyoungjin/asset/` — 나머지 전부(50개). 기존 페이지가 참조하는 25개와,
...
- 나눈 뒤 두 폴더의 파일을 MD5로 대조해, **col_chaikim이 쓰는 사진과 내용이 같으면서
  ...
  같은 내용이 `test/col_chaikim/asset/`에 이름을 바꿔 남아 있고, 지운 파일은 git 이력에도 있습니다.
...
- **`asset/collection/콜렉션 패션쇼 영상.mp4`(191MB)가 아직 지워지지 않았습니다.**
  ...
  같은 파일이 `test/col_chaikimyoungjin/asset/`에 있으므로 원본은 지워도 됩니다.
...
- `test/col_chaikim/index.html`
- `test/col_chaikim/css/col_chaikim.css` — col_chaikimyoungjin.css에서 시작
- `test/col_chaikim/js/col_chaikim.js` — col_chaikimyoungjin.js에서 시작
- `test/col_chaikim/assets/icons/arc.svg`, `arrow_right.svg` — 자매 페이지에서 복사
- `test/col_chaikim/asset/` — 사진 28장
...
- archive: 제목 `Tchai Kim`, 연도 2021 · 2020 · **2019(활성)** · 2018 · 2017 · 2015 · 2013,
  링크 `Shop Now` → `/test/shop/index.html`
- as worn: 제목 `As worn`,
...
- 자매 페이지 회귀 확인: `test/col_chaikimyoungjin/`도 이동한 `asset/`에서
...
- **이번 세션도 Browser 미리보기 패널이 화면에 표시되지 않아 스크린샷을 찍지 못했습니다.**
  ...
  **시안과 실제 화면의 눈 비교, 실제 마우스 hover, 스크롤 인터랙션의 속도감은 사용자가
  `http://localhost:5611/test/col_chaikim/index.html`을 직접 열어 확인해야 합니다.**
